...
module.exports.ListBannerImageOffline = async (req, res, next) => {
  try {
    ...
    if (getData.length > 0) {
      res.status(200).send(success("OK", { data: lastData }, res.statusCode));
    } else {
      res.status(404).send(error("OK", res.statusCode));
    }
    //res.json(dataRet);
  } catch (ex) {
    next(ex);
...
// GET LIST BANNER
module.exports.ListBannerImage = async (req, res, next) => {
  try {
